...
/**
 * Public entry point for meals. The portal has no separate meal booking:
 * meals are chosen day by day inside the room request, and only at guest
 * houses flagged `serves_meals`. The page says so rather than promising a
 * meal-only flow that does not exist.
 */
export default async function BookMealPage() {
  const [user, houses] = await Promise.all([getCurrentUser(), getSiteGuestHouses()]);
  const canBook = user !== null && REQUESTER_ROLES.includes(user.role);
  ...
  return (
    <Container className="pt-11 pb-[88px]">
      <SignInPanel
        ...
        intro={
          serving.length > 0
            ? `Sign in to place breakfast, lunch or dinner requests for your stay. Meals are chosen day by day when you request a room at ${where}.`

...
| Email in a single thread rather than standalone messages | **Done** (reworked 22 Sep 2026) | `lib/mail/thread.ts`. **One thread per booking in every mailbox** — requester, approvers and desk: sent for approval, forwarded, approved, allocated and cancelled all land in that booking's conversation. The digest, escalation and day-wise log keep a per-day **daily log** thread. Threads need both a shared root Message-ID (per booking and address, claimed by the first message sent) and an identical subject (`[reference] Guest house booking — <guest house>`). The 21 Sep version threaded staff mail per *day*, which mixed every booking of the day together; the owner asked for per-booking threads instead |
